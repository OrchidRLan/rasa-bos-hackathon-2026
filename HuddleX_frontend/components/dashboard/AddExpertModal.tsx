...
export default function AddExpertModal({
  ...
  const [userEditedWiki, setUserEditedWiki] = useState(false);
  ...
  // Auto-fill Wikipedia from name unless the user has manually edited the URL
  useEffect(() => {
    if (!userEditedWiki) {
  ...
  // Keyboard: close on Escape (disabled while distilling)
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key === "Escape" && step !== "distilling") onClose();
    }
  ...
  function handleCancel() {
    ...
    clearPhaseTimers();
    setStep("input");
    setError("Distillation cancelled.");
  }
  ...
  async function handleDistill() {
    ...
    try {
      ...
      clearPhaseTimers();
      setStep("input");
      if (abort.signal.aborted) return; // cancel is already handled by handleCancel
      const status = (e as { status?: number }).status;
      if (status === 409) {
        setError("This expert already exists in the team.");
      } else {
  ...
  return (
    ...
    <div
      ...
      onClick={(e) => {
        if (e.target === e.currentTarget && step !== "distilling") onClose();
      }}
    >
          ...
          {step === "input" && (
            <div className="space-y-4">
              ...
              <InputField
                ...
                icon={Link2}
                value={wikiUrl}
                onChange={(v) => { setWikiUrl(v); setUserEditedWiki(true); }}
          ...
          {step === "distilling" && (
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full py-2 rounded-xl border border-slate-200 text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors"
              >
                Cancel
              </button>
            </div>
