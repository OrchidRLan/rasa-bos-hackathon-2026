...
export default function ChatPanel() {
  ...
  const {
    ...
    isTranscribing,
    start: startTranscriptStream,
    stop: stopTranscriptStream,
  } = useTranscriptStream({
    sessionId,
  ...
  }, [voiceEnabled, startTranscriptStream, stopTranscriptStream]);

  const voiceStatus =
    transcriptStatus === "connecting" ? "Connecting"
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col rounded-2xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4 space-y-4">
        ...
        {voiceEnabled && transcript && transcriptStatus !== "connecting" && (
        ...
        {voiceEnabled && transcriptError && (
          <div className="flex flex-col items-end">
            <span className="text-xs text-slate-400 mb-1 px-1">Voice</span>
            <div className="max-w-[75%] px-4 py-2.5 rounded-2xl rounded-br-md bg-red-50 text-red-600 text-sm">
              {transcriptError}
            </div>
          </div>
        )}
